...
{
    uint16_t row_line = 15;
    uint16_t chunk_num = EXAMPLE_LCD_HEIGHT / row_line;
    ...
    // First fill entire screen with white
    ...
    // Now draw black borders (30 pixels wide)
    for (int i = 0; i < row_line * EXAMPLE_LCD_HEIGHT; i++) {
        color[i * byte_per_pixel + 0] = 0x00;
        color[i * byte_per_pixel + 1] = 0x00;
    }

    // Top border (30 pixels high)
    esp_lcd_panel_draw_bitmap(panel_handle, 0, 0, EXAMPLE_LCD_HEIGHT, row_line, color);
    esp_lcd_panel_draw_bitmap(panel_handle, 0, row_line, EXAMPLE_LCD_HEIGHT, 2 * row_line, color);

    // Bottom border (30 pixels high)
    esp_lcd_panel_draw_bitmap(panel_handle, 0, 22 * row_line, EXAMPLE_LCD_HEIGHT, 23 * row_line, color);
    esp_lcd_panel_draw_bitmap(panel_handle, 0, 23 * row_line, EXAMPLE_LCD_HEIGHT, 24 * row_line, color);

    // Left border (30 pixels wide)
    esp_lcd_panel_draw_bitmap(panel_handle, 0, 0, row_line, EXAMPLE_LCD_HEIGHT, color);
    esp_lcd_panel_draw_bitmap(panel_handle, row_line, 0, 2 * row_line, EXAMPLE_LCD_HEIGHT, color);

    // Right border (30 pixels wide)
    esp_lcd_panel_draw_bitmap(panel_handle, 22 * row_line, 0, 23 *row_line, EXAMPLE_LCD_HEIGHT, color);
    esp_lcd_panel_draw_bitmap(panel_handle, 23 * row_line, 0, 24 * row_line, EXAMPLE_LCD_HEIGHT, color);
